...
void main() {
	...
	//fscanf(fp, "%[^,],%[^,],%d]", Review1, Summary1, Rating1);
//	fscanf(fp, "%[^,],%[^,],%d]", Review2, Summary2, Rating2);
	...
	//for (int i = 0; i < N_Reviews; i++) {
		//printf(" good: %d bad: %d  %s tag:%d RTag: %d\n",  SGoodWords(ReviewArray[i]), SBadWords(ReviewArray[i]), ReviewArray[i].Summary.Summary,ReviewArray[i].Summary.Stag, ReviewArray[i].Rtag);
	//}
	//printf("\nPostive S: %d, Negative S: %d: PostiveR: %d  NegativeR: %d, %d ", N_postiveSummaries, N_negativeSummaries, N_postiveReviews, N_negativeReviews);
